...
import com.codex.app.adapters.FeedDmStripAdapter
...
import com.codex.app.utils.FeedRank
...
class FeedFragment : Fragment() {
    ...
    private lateinit var dmStripAdapter: FeedDmStripAdapter
    ...
    private var communityPostIds: Set<String> = emptySet()
    private var dmRooms: List<ChatRoom> = emptyList()
    private var showPrioritySections = true
    ...
    private fun setupRecycler() {
        dmStripAdapter = FeedDmStripAdapter { room ->
            startActivity(
        ...
        binding.dmStripRecycler.layoutManager =
            LinearLayoutManager(requireContext(), LinearLayoutManager.HORIZONTAL, false)
        binding.dmStripRecycler.adapter = dmStripAdapter
        ...
        postAdapter = PostAdapter(
            ...
            communityPostIds = { communityPostIds }
        )
    ...
    private fun applyFeedFilter() {
        ...
        val favFiltered = filterPosts(favoritePosts, query)
        val allFiltered = filterPosts(allPosts, query)
        val display = if (showPrioritySections) favFiltered + allFiltered else allFiltered
        postAdapter.submitList(display)
        updatePriorityHeaders(favFiltered, allFiltered)
        binding.emptyText.visibility = if (display.isEmpty() && dmRooms.isEmpty()) View.VISIBLE else View.GONE
    }

    private fun updatePriorityHeaders(favFiltered: List<Post>, allFiltered: List<Post>) {
        if (!showPrioritySections) {
            binding.feedMessagesHeader.visibility = View.GONE
            binding.dmStripRecycler.visibility = View.GONE
            binding.feedCommunitiesHeader.visibility = View.GONE
            binding.feedAllPostsHeader.visibility = View.GONE
            return
        }

        val showDm = dmRooms.isNotEmpty()
        binding.feedMessagesHeader.visibility = if (showDm) View.VISIBLE else View.GONE
        binding.dmStripRecycler.visibility = if (showDm) View.VISIBLE else View.GONE

        binding.feedCommunitiesHeader.visibility =
            if (favFiltered.isNotEmpty()) View.VISIBLE else View.GONE

        binding.feedAllPostsHeader.visibility =
            if (showDm || favFiltered.isNotEmpty()) View.VISIBLE else View.GONE
    }
    ...
    private fun loadPosts() {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            ...
            showPrioritySections = currentCategory == FirebaseHelper.ALL_CATEGORY_LABEL
            ...
            if (showPrioritySections) {
                ...
                dmRooms = if (uid != null) FirebaseHelper.getRecentDmRooms() else emptyList()
                dmStripAdapter.submitList(dmRooms)

                val favorites = uid?.let { FirebaseHelper.getFavoriteCategories(it) } ?: emptyList()
                val favoriteNames = favorites.map { it.name.lowercase() }.toSet()

                result.onSuccess { posts ->
                    val partitioned = FeedRank.partitionFeedPosts(posts, favoriteNames)
                    favoritePosts = partitioned.favoritePosts
                    allPosts = partitioned.allPosts
                    communityPostIds = favoritePosts.map { it.id }.toSet()
                }.onFailure {
                    favoritePosts = emptyList()
                    allPosts = emptyList()
                    communityPostIds = emptySet()
                    dmRooms = emptyList()
                    dmStripAdapter.submitList(emptyList())
                }
            } else {
                dmRooms = emptyList()
                dmStripAdapter.submitList(emptyList())
            ...
            if (favoritePosts.isEmpty() && allPosts.isEmpty()) {
