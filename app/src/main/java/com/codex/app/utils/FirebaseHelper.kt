...
object FirebaseHelper {
    ...
    suspend fun getPosts(
        ...
        return try {
            ...
            val blocked = getBlockedUserIds()
            if (blocked.isNotEmpty()) {
                posts = posts.filter { it.authorId !in blocked }
            }
            Result.success(posts.take(limit.toInt()))
        } catch (e: Exception) {
            Log.e(TAG, "getPosts error", e)
            val err = if (e is FirebaseFirestoreException) mapFirestoreError(e) else e
            Result.failure(err)
        }
    }
    ...
    suspend fun getFeedHiddenTopicNames(): Set<String> {
        return try {
            ...
            Log.w(TAG, "getFeedHiddenTopicNames fallback", e)
            emptySet()
        }
    }
    ...
    private suspend fun getBlockedUserIds(): Set<String> {
        ...
        return try {
            val snap = db.collection(USERS).document(uid).collection(BLOCKED_USERS).get().await()
            snap.documents.map { it.id }.toSet()
        } catch (e: Exception) {
            Log.w(TAG, "getBlockedUserIds fallback", e)
            emptySet()
        }
    }
    ...
    suspend fun getFeedCategoryNames(includeFeedHidden: Boolean = false): List<String> {
        ...
        val visible = fromDb
            .filter { !hidden.contains(it.lowercase()) }
    ...
    suspend fun deleteComment(commentId: String, postId: String): Result<Unit> {
        ...
        val actorRole = currentUser?.getRoleEnum()
            ?: fetchUser(uid)?.getRoleEnum()
            ?: Role.USER
        return try {
            val commentSnap = db.collection(COMMENTS).document(commentId).get().await()
            val comment = commentSnap.toObject(Comment::class.java)
                ?: return Result.failure(Exception("Comment not found."))
            val postAuthorId = db.collection(POSTS).document(postId).get().await()
                .getString("authorId")
            val canDelete = actorRole.canModerate() ||
                actorRole.isFounder() ||
    ...
    suspend fun searchPosts(query: String, limit: Int = 30): List<Post> {
        ...
        return getPosts(limit = 200).getOrElse { emptyList() }.filter { post ->
