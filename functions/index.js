...
async function removeUserFromOthersFriendLists(targetUid) {
  ...
  const refs = snap.docs.filter((doc) => doc.id === targetUid).map((doc) => doc.ref);
  await commitBatchDeletes(refs);
}
...
exports.deleteUserAccount = onCall({ cors: true }, async (request) => {
  try {
    ...
    await assertFounder(callerUid, callerEmail);
    ...
    const anonymizedPosts = await anonymizeAuthorContent('posts', targetUid);
    const anonymizedComments = await anonymizeAuthorContent('comments', targetUid);

    await deleteFriendRequestsForUser(targetUid);
    await removeUserFromOthersFriendLists(targetUid);
    await cleanupChessGamesForUser(targetUid);
    await endVoiceSessionsForUser(targetUid);
    ...
    await deleteUserStorage(targetUid);
