...
const FOUNDER_EMAIL = 'ericdanielevans@gmail.com';
...
exports.onPostReactionCreated = onDocumentCreated('posts/{postId}/reactions/{reactionId}', async (event) => {
  const reaction = event.data?.data();
  if (!reaction) return;

  const postId = event.params.postId;
  const reactorUid = reaction.userId;
  ...
  const post = postSnap.data();
  const authorId = post.authorId;
  if (!authorId || authorId === reactorUid) return;

  const reactor = await fetchUser(reactorUid);
...
function isFounderAccount(userData, email) {
  const role = String(userData?.role || '').toUpperCase();
  return role === 'FOUNDER' || normalizeEmail(email) === FOUNDER_EMAIL;
}

async function assertFounder(uid, authEmail) {
  const user = await fetchUser(uid);
  if (isFounderAccount(user, user?.email || authEmail)) return;
...
async function anonymizeAuthorContent(collectionName, targetUid) {
  let count = 0;
  let lastDoc = null;
  // Paginate with startAfter — authorId stays the same after anonymize updates.
  // eslint-disable-next-line no-constant-condition
  while (true) {
...
async function deleteUserSubcollections(targetUid) {
  ...
  for (const name of subcollections) {
    await deleteQueryInBatches(db.collection('users').doc(targetUid).collection(name));
  }
}

async function deleteFriendRequestsForUser(targetUid) {
  const fromSnap = await db.collection('friendRequests').where('fromUid', '==', targetUid).get();
  const toSnap = await db.collection('friendRequests').where('toUid', '==', targetUid).get();
  const refs = [...fromSnap.docs, ...toSnap.docs].map((d) => d.ref);
  await commitBatchDeletes(refs);
}

async function removeUserFromOthersFriendLists(targetUid) {
  const refs = [];
  let lastDoc = null;
  // Paginate — a single collectionGroup().get() can timeout on large datasets.
  // eslint-disable-next-line no-constant-condition
  while (true) {
    let query = db.collectionGroup('friends').limit(BATCH_SIZE);
    if (lastDoc) query = query.startAfter(lastDoc);
    const snap = await query.get();
    if (snap.empty) break;
    refs.push(...snap.docs.filter((doc) => doc.id === targetUid).map((doc) => doc.ref));
    lastDoc = snap.docs[snap.docs.length - 1];
    if (snap.size < BATCH_SIZE) break;
  }
  await commitBatchDeletes(refs);
...
async function runDeleteStep(label, fn) {
  try {
    return await fn();
  } catch (err) {
    if (err instanceof HttpsError) throw err;
    console.error(`deleteUserAccount: ${label}`, err);
    throw new HttpsError('internal', `${label}: ${err?.message || err}`);
  }
}

async function cleanupChessGamesForUser(targetUid) {
  const asWhite = await db.collection('chessGames').where('whiteUid', '==', targetUid).get();
  const asBlack = await db.collection('chessGames').where('blackUid', '==', targetUid).get();
  const seen = new Set();
  const docs = [...asWhite.docs, ...asBlack.docs].filter((doc) => {
  ...
  docs.forEach((doc) => {
    ...
    if (status === 'pending') {
      batch.update(doc.ref, { status: 'declined', updatedAt: Timestamp.now() });
...
async function endVoiceSessionsForUser(targetUid) {
  const active = await db.collection('voiceSessions')
  ...
  for (const doc of active.docs) {
    ...
    const involved =
      data.createdBy === targetUid ||
      data.calleeUid === targetUid ||
      targetUid in participants;
...
async function deleteUserStorage(targetUid) {
  try {
    const bucket = getStorage().bucket();
    await bucket.deleteFiles({ prefix: `profile_pics/${targetUid}/` }).catch(() => {});
    await bucket.deleteFiles({ prefix: `profile_backgrounds/${targetUid}/` }).catch(() => {});
  } catch (err) {
    console.warn('deleteUserStorage', err?.message || err);
  }
}
...
exports.deleteUserAccount = onCall(
  ...
  async (request) => {
    try {
      ...
      await runDeleteStep('Founder check', () => assertFounder(callerUid, callerEmail));
      ...
      const targetProfile = await fetchUser(targetUid);
      let targetEmail = normalizeEmail(targetProfile?.email);
      try {
        const authRecord = await getAuth().getUser(targetUid);
        targetEmail = targetEmail || normalizeEmail(authRecord.email);
      } catch (err) {
        if (err?.code === 'auth/user-not-found' && !targetProfile) {
      ...
      if (targetEmail === FOUNDER_EMAIL) {
        throw new HttpsError('failed-precondition', 'The founder account cannot be deleted.');
      }

      const anonymizedPosts = await runDeleteStep('Post anonymization', () =>
        anonymizeAuthorContent('posts', targetUid)
      );
      const anonymizedComments = await runDeleteStep('Comment anonymization', () =>
        anonymizeAuthorContent('comments', targetUid)
      );

      await runDeleteStep('Friend request cleanup', () => deleteFriendRequestsForUser(targetUid));
      await runDeleteStep('Friend list cleanup', () => removeUserFromOthersFriendLists(targetUid));
      await runDeleteStep('Chess game cleanup', () => cleanupChessGamesForUser(targetUid));
      await runDeleteStep('Voice session cleanup', () => endVoiceSessionsForUser(targetUid));
      await runDeleteStep('Profile subcollection cleanup', () => deleteUserSubcollections(targetUid));

      if (targetProfile) {
        await runDeleteStep('Profile document delete', () =>
          db.collection('users').doc(targetUid).delete()
        );
      }

      await runDeleteStep('Storage cleanup', () => deleteUserStorage(targetUid));

      await runDeleteStep('Auth user delete', async () => {
        try {
